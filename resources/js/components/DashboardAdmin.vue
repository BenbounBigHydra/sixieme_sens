  ...
  <div class="min-h-screen bg-[#fffbf1]">
    ...
    <main class="max-w-[1200px] mx-auto py-10 px-4 space-y-12">
      ...
      <section class="pt-8">
        ...
        <div class="relative h-[300px] w-full border-l border-b border-black">
          ...
          <div v-for="i in (maxChartValue / 2)" :key="'y-'+i" class="absolute -left-8 text-xs font-bold font-inter text-right w-6" :style="{ bottom: `${(i / (maxChartValue / 2)) * 100 - 2}%` }">
            {{ i * 2 }}
          </div>
          ...
          <div class="absolute inset-0 flex items-end justify-around px-4">
            <div v-for="(month, i) in computedMonthsData" :key="i" class="w-16 h-full flex flex-col justify-end items-center relative">
              <div :class="[month.color, 'w-full cursor-pointer']" 
                   ...
                   @mouseleave="hoveredMonthIndex = null"></div>
              <div class="absolute top-[105%] text-xs font-bold font-inter whitespace-nowrap" style="writing-mode: vertical-rl; transform: rotate(180deg);">
                {{ month.label }}
              </div>
            </div>
          </div>
          ...
          <div v-if="hoveredMonthIndex !== null" class="absolute top-[105%] left-0 w-full bg-[#fffbf1] border border-black z-40 shadow-[4px_4px_0px_0px_rgba(0,0,0,1)]">
            <div class="p-4">
              ...
              <div v-if="computedMonthsData[hoveredMonthIndex].collections.length === 0" class="text-sm font-inter text-gray-500 py-4">
              ...
              <div v-else class="relative w-full h-[120px]">
                ...
                <!-- Events -->
                <div v-for="(col, index) in computedMonthsData[hoveredMonthIndex].collections" :key="col.id || index" 
                   ...
                   <span class="text-[10px] font-inter text-black absolute whitespace-nowrap" :class="index % 2 === 0 ? 'bottom-[55px]' : 'bottom-[85px]'">
                   ...
                   <div class="w-[1px] bg-black absolute bottom-0" :class="index % 2 === 0 ? 'h-[25px]' : 'h-[55px]'"></div>
                </div>
              </div>
            </div>
          </div>
      ...
      <section class="grid grid-cols-2 gap-12 mt-8">
        ...
        <div class="border border-black flex flex-col">
          ...
          <div class="p-8 flex flex-col items-center flex-grow">
            <div class="font-jersey text-6xl text-black">{{ participationRatio.globalAverage !== undefined ? (participationRatio.globalAverage * 100).toFixed(0) + '%' : '21%' }}</div>
            <div class="font-jersey text-sm text-black mb-8">De participation moyenne</div>

            <div class="w-full space-y-4 font-inter text-sm">
              <div v-for="(item, i) in participationRatio.lowestCompanies || mockParticipationCompanies" :key="i" class="flex justify-between items-center border-b border-gray-300 pb-2">
                ...
                <div class="font-bold">{{ item.ratio !== undefined ? (item.ratio * 100).toFixed(0) + '%' : item.percentage }}</div>
              </div>
            </div>
          </div>
        </div>
        ...
        <div class="border border-black flex flex-col">
          ...
          <div class="p-8 flex flex-col items-center flex-grow">
            <div class="font-jersey text-6xl text-black">{{ rigorRatio.globalAverage !== undefined ? (rigorRatio.globalAverage * 100).toFixed(0) + '%' : '80%' }}</div>
            <div class="font-jersey text-sm text-black mb-8">De rigueur moyenne</div>

            <div class="w-full space-y-4 font-inter text-sm">
              <div v-for="(item, i) in rigorRatio.lowestCompanies || mockRigorCompanies" :key="i" class="flex justify-between items-center border-b border-gray-300 pb-2">
                <div class="flex items-center space-x-3">
                  <img v-if="item.warning" src="/images/importantOrange.svg" alt="Warning" class="h-5" />
                  <div v-else class="h-5 w-5"></div>
                  <span>{{ item.company?.name || item.name }}</span>
    ...
    <div v-if="activeModal" class="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4" @click.self="closeModal">
      ...
      <div v-if="activeModal === 'cloture'" class="bg-[#fffbf1] border-[10px] border-[#5C629E] w-full max-w-[600px] p-8 relative">
        ...
        <h2 class="font-['Jersey_20'] text-5xl text-black leading-none mb-1">{{ selectedCollecte?.company?.name || 'HEIG-VD' }}</h2>
        <p class="font-['Jersey_20'] text-2xl text-black mb-8 tracking-wide">Collecte du {{ formatDate(selectedCollecte?.day_start) || '02.03.2026' }}</p>

        <div class="space-y-6 mb-8">
          <div>
            <label class="block font-inter text-base mb-2">Nombre d'inscrits</label>
            <div class="relative">
              <input type="number" v-model="clotureForm.nb_registered" placeholder="Placeholder" class="w-full border border-black p-3 font-inter text-sm bg-transparent" />
          ...
          <div>
            ...
            <div class="relative">
              <input type="number" v-model="clotureForm.nb_blood_pouch" placeholder="Placeholder" class="w-full border border-black p-3 font-inter text-sm bg-transparent" />
              <span class="absolute right-3 top-1/2 -translate-y-1/2">*</span>
            </div>
          </div>
        </div>
        ...
        <div class="flex space-x-4">
          ...
          <button @click="submitCloture" class="bg-[#5bb124] text-white px-6 py-2.5 flex items-center justify-center font-inter font-medium w-32 hover:bg-green-600">
      ...
      <div v-if="activeModal === 'detail'" class="bg-[#fffbf1] border-[10px] border-[#0073e6] w-full max-w-[750px] p-8 relative">
        ...
        <h2 class="font-['Jersey_20'] text-5xl text-black leading-none mb-1">{{ selectedCollecte?.company?.name || 'HEIG-VD' }}</h2>
        <p class="font-['Jersey_20'] text-2xl text-black mb-8 tracking-wide">Collecte du {{ formatDate(selectedCollecte?.day_start) || '02.07.2026' }}</p>
        ...
        <div class="flex justify-between items-center">
          ...
          <div class="flex space-x-4">
            ...
            <button @click="submitDetail" class="bg-[#0073e6] text-white px-8 py-3 font-inter font-medium hover:bg-blue-600 transition-colors">Enregistrer</button>
          </div>
        </div>
      </div>
      ...
      <div v-if="activeModal === 'suppression'" class="bg-[#fffbf1] border-[10px] border-[#E4534B] w-full max-w-[650px] p-10 relative">
        ...
        <p class="font-inter text-lg text-black mb-8 leading-relaxed">
          Vous êtes sur le point de supprimer la collecte de {{ selectedCollecte?.company?.name || 'HEIG-VD' }} du {{ formatDate(selectedCollecte?.day_start) || '02.03.2026' }}.<br/>
        ...
        <div class="flex justify-end mt-4">
          <button @click="submitSuppression" class="bg-[#E4534B] text-white px-8 py-3 font-inter text-lg font-medium hover:bg-red-600 transition-colors">Supprimer</button>
        </div>
      </div>
  ...
  methods: {
    ...
    openModal(type, item) {
      ...
      if (type === 'cloture') {
        ...
        this.detailForm.day_start = item?.day_start ? item.day_start.split(' ')[0] : '';
